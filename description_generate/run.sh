...
do
    ...
    CUDA_VISIBLE_DEVICES=$gpu_id nohup python3 /root/patch_matters_re-18/description_generate/multi_process.py \
        --input_file /root/patch_matters_re-18/description_generate/test_data/did_bench.json \
        --output_folder /root/patch_matters_re-18/description_generate \
        --chunk_index $chunk_index \
        --chunk_num $chunk_num \
        --node_index $node_index \
        --node_num $node_num > /root/patch_matters_re-18/description_generate/test_$chunk_index.log 2>&1 &
done

wait

python3 /root/patch_matters_re-18/description_generate/combine.py \
    --folder_path /root/patch_matters_re-18/description_generate \
    --output_file /root/patch_matters_re-18/description_generate/description_output.json
